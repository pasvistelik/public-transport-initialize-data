        ...
        try {
            ...
            for (var index = 0, tmpArr = [], tabArr = []; index <= 1; index = 1) {
                ...
                for (var j = 0, m = currentRouteStationsCodes[index].length, stationCode = currentRouteStationsCodes[index][0]; j < m; stationCode = currentRouteStationsCodes[index][++j]) {
                    var tmpUsed = false;
                    for (var k = 0, mn = tmpUsedStations.length, station = tmpUsedStations[0]; k < mn; station = tmpUsedStations[++k]) {
                    ...
                    if (!tmpUsed) {
                        for (var _k = 0, _mn = allStations.length, _station = allStations[0]; _k < _mn; _station = allStations[++_k]) {
                            if (_station != null && _station.hashcode === stationCode) {
                                bindRoutesStationsTimetables(_station, tmpArr, tabArr, currentRoute);
