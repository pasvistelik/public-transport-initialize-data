...
function findTimeAfter(time) {
    ...
    for (var kkk = 0, mnkk = this.table.length, t = this.table[0]; kkk < mnkk; t = this.table[++kkk]) {
        if (t.days.includes(day)) {
            ...
            var findedTime = binaryFind(t.times, function (array, item, index) {
                var stTime = item.hour * 3600 + item.minute * 60;
                if (stTime >= time && (array[index + 1] == null || array[index + 1].hour * 3600 + array[index + 1].minute * 60 < time)) return 0;else if (stTime >= time) return 1;else return -1;
            });
            ...
            for (var index = 0, tmpArr = [], tabArr = []; index <= 1; index = 1) {
                ...
                var _loop = function _loop(j, m, stationCode) {
                    ...
                    if (!tmpUsed) {
                        ...
                        var findedStation = binaryFind(allStations, function (array, item, index) {
                            if (item.hashcode === stationCode) return 0;else if (item.hashcode > stationCode) return 1;else return -1;
                        });
