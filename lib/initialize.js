...
function getNextStation(currentStation) {
    for (var j = 0, thisStations = this.stations[0], nn = thisStations.length - 1; j <= 1; thisStations = this.stations[j = 1], nn = thisStations.length - 1) {
...
function getPreviousStation(currentStation) {
    for (var j = 0, thisStations = this.stations[0]; j <= 1; thisStations = this.stations[j = 1]) {
        ...
        for (var t = 1, nn = thisStations.length; t < nn; t++) {
...
function findTimeAfter(time, dayToday) {
    ...
    for (var kkk = 0, mnkk = this.table.length, t = this.table[0]; kkk < mnkk; t = this.table[++kkk]) {
        if (t.days[day]) {
            ...
            var findedTime = binaryFind(t.times, findingTimeAfterFunc);
...
function findTimeBefore(time, dayToday) {
    ...
    for (var kkk = 0, mnkk = this.table.length, t = this.table[0], ok = false, st; kkk < mnkk; t = this.table[++kkk]) {
        if (t.days[day]) {
            ...
            var findedTime = binaryFind(t.times, findingTimeBeforeFunc);
            if (findedTime != null) return findedTime - time;
    ...
    for (var _i = 0, _n = allRoutes.length, currentRoute = allRoutes[0]; _i < _n; currentRoute = allRoutes[++_i]) {
        ...
        try {
            ...
            for (var index = 0, tmpArr = [], tabArr = []; index <= 1; index++) {
                ...
                for (var j = 0, m = currentRouteStationsCodes[index].length, stationCode = currentRouteStationsCodes[index][0], findingStationFunc = getFindingStationFunc(stationCode); j < m; stationCode = currentRouteStationsCodes[index][++j], findingStationFunc = getFindingStationFunc(stationCode)) {

                    var findedStation = binaryFind(allStations, findingStationFunc);
